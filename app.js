...
function initApp() {
  try {
    ...
    stateRef.on("value", snap=>{
      const val = snap.val();
      if (!val && !_isResetting) stateRef.set(INIT_STATE());
      APP = ensureStateShape(val);
...
function shopReset() {
  // Must use a body-level modal — render() rewrites s-body every second
  // so any inline confirm box would flash and disappear immediately.
  ...
  modal.innerHTML =
    '<div id="reset-confirm-box" style="' +
      'background:linear-gradient(155deg,#1e0a0a,#1e1e1e);' +
      'border:2px solid rgba(255,80,80,.4);border-radius:20px;' +
      'padding:36px 28px 28px;max-width:380px;width:100%;' +
      'box-shadow:0 0 80px rgba(255,60,60,.12),0 30px 60px rgba(0,0,0,.6);' +
      'transform:translateY(18px) scale(.96);transition:transform .22s ease;' +
      'text-align:center;">' +
      '<div style="font-size:44px;margin-bottom:12px">&#x26A0;&#xFE0F;</div>' +
      '<div style="font-size:19px;font-weight:800;color:#ff6060;margin-bottom:8px">Reset All Data?</div>' +
      '<div style="color:#aaa;font-size:13px;line-height:1.65;margin-bottom:24px">' +
        'This will clear all sessions, logs, and reservations.<br>' +
        '<strong style="color:#ff8080">This cannot be undone.</strong>' +
      '</div>' +
      '<div style="display:flex;gap:10px">' +
        '<button class="btn btn-ghost" style="flex:1;padding:13px;font-size:14px"' +
          ' onclick="document.getElementById(\'reset-confirm-modal\').remove()">Cancel</button>' +
        '<button class="btn btn-danger" style="flex:1;padding:13px;font-size:14px;font-weight:700"' +
          ' onclick="document.getElementById(\'reset-confirm-modal\').remove();confirmReset()">' +
  ...
  requestAnimationFrame(function() {
    modal.style.opacity = "1";
    var box = document.getElementById("reset-confirm-box");
    if (box) box.style.transform = "translateY(0) scale(1)";
  });
}
...
function confirmReset() {
  _isResetting = true;
  var fresh = INIT_STATE();
  if (isFirebase && db) {
    stateRef.set(fresh);
    db.ref(RESERVATION_REF).remove();
    db.ref(NOTIFICATIONS_REF).remove();
    db.ref(REVENUE_RESET_REF).remove();
  } else {
  ...
  APP = fresh;
  notifications = [];
  revenueResetData = null;
  reservations = [];
  activeAlertIds.clear();
  alertedPlayedIds.clear();
  stopAlert();
  setTimeout(function() { _isResetting = false; }, 3000);
  toast("System reset complete. All sessions and logs cleared.","success");
  render();
}
...
// 1. Mag-upload ng larawan sa Firebase Storage
//    path: profile_pictures/{username}
// 2. Kuhanin ang download URL
// 3. I-save ang URL sa Realtime Database: accounts/{username}/avatarUrl
function handleAvatarUpload(event) {
  const file = event.target.files[0];
  if (!file) return;

  // 5MB limit (Firebase Storage kaya na malaking files)
  if (file.size > 5 * 1024 * 1024) {
    toast("Image too large. Max 5MB lang.","warn");
    event.target.value = "";
    return;
  }

  // Show loading state sa avatar
  const display = document.getElementById("profile-avatar-display");
  if (display) {
    display.innerHTML = `<div style="font-size:12px;color:var(--gr);text-align:center;padding:8px">⏳<br>Uploading...</div>`;
  }
  ...
  try {
    ...
    const storageRef  = storage.ref("profile_pictures/" + USER.username);

    // I-upload ang file
    const uploadTask = storageRef.put(file);

    uploadTask.on("state_changed",
      // Progress (optional — pwedeng dagdagan ng progress bar)
      (snapshot) => {
        const pct = Math.round((snapshot.bytesTransferred / snapshot.totalBytes) * 100);
        if (display) display.innerHTML = `<div style="font-size:12px;color:var(--gr);text-align:center;padding:8px">⏳<br>${pct}%</div>`;
      },
      // Error
      (err) => {
        console.error("Upload error:", err);
        toast("Upload failed: " + err.message,"warn");
        // Ibalik ang dating avatar
        render();
      },
      // Success — kuhanin ang download URL
      () => {
        uploadTask.snapshot.ref.getDownloadURL().then((downloadURL) => {
          // I-save ang URL sa Realtime Database
          db.ref(ACCOUNTS_REF + "/" + USER.username + "/avatarUrl").set(downloadURL)
            .then(() => {
              USER.avatarUrl = downloadURL;
              persistSession();
              toast("✅ Profile photo updated!","success");
              // I-update ang avatar display agad
              if (display) {
                display.innerHTML = `<img src="${downloadURL}" style="width:100%;height:100%;object-fit:cover;">`;
              }
            })
            .catch(() => toast("Photo uploaded pero hindi nasave ang URL. Try again.","warn"));
        });
      }
    );
  } catch (e) {
    console.error(e);
    toast("Firebase Storage hindi available.","warn");
    render();
  }

  event.target.value = ""; // reset para pwedeng same file ulit
}
